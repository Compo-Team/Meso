...
class Meso extends Base
{
    ...
    public function readList(string $table, int $offset = 0)
    {
    	if(!preg_match("/[`]/", $table))
    	{
    		$table = "`".$table."`";
    	}
		$query = "SELECT * FROM {$table} ORDER BY `Date` LIMIT {$offset}";
    	if($offset == 0)
    	{
	...
	public function update(string $table, object $object, array $array)//not done yet.
	{
		if(!preg_match("/[`]/", $table))
    	{
    		$table = "`".$table."`";
    	}
		$values = self::andFormat($array, ", ");
		$object_array = (array) $object;
    	$rowID = self::andFormat($object_array);
